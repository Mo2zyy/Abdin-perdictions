...
async function loadMatches() {
  ...
  try {
    ...
    matches.forEach(m => {
      ...
      const p = map[m.id] || {};
      const homeLogo = TEAM_LOGOS[m.home_team] ? `<img src="${TEAM_LOGOS[m.home_team]}" style="width:24px; height:24px; vertical-align:middle; margin:0 124px;">` : '';
      const awayLogo = TEAM_LOGOS[m.away_team] ? `<img src="${TEAM_LOGOS[m.away_team]}" style="width:24px; height:24px; vertical-align:middle; margin:0 124px;">` : '';
