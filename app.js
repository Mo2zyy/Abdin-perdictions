...
// ==========================================
// ملف app.js - النسخة الشاملة (حفظ + جلب النتائج الحقيقية والاسكورات)
// ==========================================
...
// خريطة الهيكل المتوافقة مع IDs صفحة الـ HTML
const matchesStructure = [
  { r: 1, m: 1, home: 'Desoky', away: 'Seif' },
  { r: 1, m: 2, home: 'Angle', away: 'Eslam' },
  { r: 1, m: 3, home: 'Loay', away: 'Ahd' },
...
async function loadUserPredictions() {
  ...
  try {
    ...
    // 1. جلب توقعات المستخدم من Supabase
    const { data: userPreds, error } = await db.from('predictions').select('*').eq('username', currentUser);
    if (error) {
      console.error('خطأ في جلب التوقعات:', error);
      return;
    ...
    if (matches) {
      matches.forEach(m => {
        matchesDict[m.id] = m;
        // الاحتياط لو الـ id مخزن بشكل مختلف
        if (m.match_id) matchesDict[m.match_id] = m;
      });
    }

    if (userPreds) {
      userPreds.forEach((p, index) => {
        let matchIndex = (p.match_id !== undefined && p.match_id !== null) ? p.match_id - 1 : index;
        let struct = matchesStructure[matchIndex] || 
                     matchesStructure.find(s => s.home === p.home_team && s.away === p.away_team);
        ...
        if (struct) {
          ...
          // وضع اسكوار التوقع في الحقول
          if (hInput && p.predicted_home_score !== null) hInput.value = p.predicted_home_score;
          if (aInput && p.predicted_away_score !== null) aInput.value = p.predicted_away_score;

          // جلب النتيجة الحقيقية المطابقة
          const realMatch = matchesDict[p.match_id] || matchesDict[matchIndex + 1] || matchesDict[struct.r];
          ...
          if (realMatch && realMatch.home_score !== null && realMatch.home_score !== undefined && 
            ...
            let row = hInput ? hInput.closest('.match-row') : null;
            if (row) {
              let badge = row.querySelector('.result-badge');
              if (!badge) {
                badge = document.createElement('div');
...
// دالة حفظ التوقعات عند قيام المستخدم بإدخالها وضغط حفظ
async function savePredictions() {
  if (!currentUser) {
    alert('من فضلك سجل دخولك بالاسم أولاً!');
    return;
  }
  if (!db) {
  ...
  try {
    let savedCount = 0;
    for (let i = 0; i < matchesStructure.length; i++) {
      let struct = matchesStructure[i];
      let matchId = i + 1;
      ...
      if (hInput && aInput && hInput.value !== '' && aInput.value !== '') {
        const predData = {
          ...
          match_id: matchId,
          home_team: struct.home,
          away_team: struct.away,
          predicted_home_score: parseInt(hInput.value),
          predicted_away_score: parseInt(aInput.value)
        };

        // رفع او تحديث التوقع في Supabase
        const { error } = await db.from('predictions')
          .upsert(predData, { onConflict: 'username,match_id' });

        if (error) {
          console.error('خطأ في حفظ التوقع للماتش رقم', matchId, error);
        } else {
          savedCount++;
        }
      }
    ...
    alert(`تم حفظ ${savedCount} توقع بنجاح! 🚀`);
    loadUserPredictions(); // إعادة تحميل لجلب النتائج والنقاط
  } catch (err) {
    console.error('Error saving predictions:', err);
    حدث خطأ أثناء حفظ التوقعات.
  }
}
